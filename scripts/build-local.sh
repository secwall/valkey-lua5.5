...
case "$PKG_TYPE" in
    ...
    rpm)
        ...
        ARCH=$(uname -m)
        PKG_NAME="valkey-lua55-nightly"
        if [[ "$VERSION" != *-dev* ]]; then
            PKG_NAME="valkey-lua55"
        fi
