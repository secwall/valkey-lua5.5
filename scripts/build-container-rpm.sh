...
#   PKG_NAME         — "valkey-lua55" or "valkey-lua55-nightly"
#
# Expected mounts:
...
sed -e "s/#\[RPM_VERSION\]/${RPM_VERSION}/g" \
    ...
    /packaging/valkey-lua55.spec.in > ~/rpmbuild/SPECS/${PKG_NAME}.spec
...
cp /source/valkey-lua55-${MODULE_VERSION}.tar.gz ~/rpmbuild/SOURCES/
...
if ! echo "$RPM_CONTENTS" | grep -q 'libvalkeylua55.so'; then
    echo "ERROR: libvalkeylua55.so not found in RPM!" >&2
    exit 1
fi
