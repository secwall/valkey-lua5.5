...
#
# Expected mounts:
#   /source    — source tarball (valkey-lua55-VERSION.tar.gz)
#   /packaging — packaging/ directory from repo
#   /scripts   — scripts/ directory from repo
#   /output    — directory for built DEBs
set -euo pipefail

echo "==> Building DEB for valkey-lua55 ${MODULE_VERSION} on ${PLATFORM_ID} (${EXPECTED_ARCH})"
...
tar xzf /source/valkey-lua55-${MODULE_VERSION}.tar.gz
SRCDIR="$BUILDDIR/valkey-lua55-${MODULE_VERSION}"
cd "$SRCDIR"
...
cp /source/valkey-lua55-${MODULE_VERSION}.tar.gz \
   "$BUILDDIR/valkey-lua55_${DEB_VERSION}.orig.tar.gz"
...
DEB_FILE=$(find "$BUILDDIR" -maxdepth 1 -name "valkey-lua55_*.deb" ! -name "*-dbgsym*" | head -1)
if [ -z "$DEB_FILE" ]; then
...
if ! echo "$DEB_CONTENTS" | grep -q 'libvalkeylua55.so'; then
    echo "ERROR: libvalkeylua55.so not found in DEB!" >&2
    exit 1
fi
...
cp "$BUILDDIR"/valkey-lua55_*.deb /output/
cp "$BUILDDIR"/valkey-lua55_*.changes /output/ 2>/dev/null || true
cp "$BUILDDIR"/valkey-lua55_*.buildinfo /output/ 2>/dev/null || true

echo "==> Output:"
ls -la /output/valkey-lua55*
